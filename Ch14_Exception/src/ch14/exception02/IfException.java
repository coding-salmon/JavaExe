 ...
 *  	필요하면 사용자 추가 정의
 *  
 *  예전에는 if문으로 프로그래밍 처리 -> 에러 확인 -> 연산 처리
 *  이렇게 프로그래밍을 했다.
 *  
 *  입력등에 예상치 못한 일이 많이 발생하므로
 *  프로그램에서 '오류 처리'가 '프로그래밍 로직'보다
 *  점점 커지는 현상이 일어났다.
 *  => 코드의 가독성 저ㅏ
 *  => 지저분해 보이는 현상
 *  => 비용이 증가
 ...
 *  
 *  }catch{
 *  //try내에서 로직 실행 중 발생한 모든 예외를 처리하는 곳
 *  }
 *  
 *  Java는 JDK라는 라이브러리를 제공하므로 직접 여러 모듈/클래스/메서드 설계했다.
 *  설계 과정에서 일어날 수 있는 모든 Error에 대해서 '미리 정의된 Exception'을
 *  제공함으로써 개발자가 손쉽게 예외처리 할 수 있도록 도와준다
 *  물론 모든 것으 예측할 수 없으므로 개발자가 직접 작성해야 할 때도 있다.
 *  
 */
