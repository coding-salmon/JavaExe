...
public class EmployeeManager {
	...
	private void modifyEmployee(Employee emp) { //사번으로 수정하는 메서드
		if (emp instanceof RegularEmployee) {
	        ...
	        ((TempEmployee) emp).setHireYear(newHireYear);
	...
	private List<Employee> empArr = new LinkedList<Employee>();
	...
	private void viewAllEmployeeInfo() {
		for(Employee emp : this.empArr) { // empArr을 emp 변수에 할당 
			emp.showEmployeeInfo();
		}
	}
	private void viewRegEmployeeInfo() {
		for(Employee emp : this.empArr) {
			if(emp instanceof RegularEmployee)
				emp.showEmployeeInfo();
		}
	}
